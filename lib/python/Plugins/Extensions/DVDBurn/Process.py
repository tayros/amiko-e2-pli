...
class DVDJob(Job):
	...
	def conduct(self):
		...
		if self.menupreview:
			...
			burnargs += [ "-dvd-video", "-publisher", "set-top-box", "-V", volName, self.workspace + "/dvd" ]
...
class DVDdataJob(Job):
	...
	def conduct(self):
		...
		burnargs += [ "-publisher", "set-top-box", "-V", volName, "-follow-links", self.workspace ]
		BurnTask(self, burnargs, tool)
		RemoveDVDFolder(self)
...
class DVDisoJob(Job):
	def __init__(self, project, imagepath):
		...
		if imagepath.endswith(".iso"):
			...
			if getSize(imagepath)/(1024*1024) > self.project.MAX_SL:
				burnargs += [ "-use-the-force-luke=4gms", "-speed=1", "-R" ]
			burnargs += [ "-dvd-video", "-publisher", "set-top-box", "-V", volName, imagepath ]
		tool = "growisofs"
		BurnTask(self, burnargs, tool)
