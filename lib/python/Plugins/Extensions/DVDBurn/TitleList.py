...
class TitleList(Screen, HelpableScreen):
	...
	def selectedSource(self, source = None):
		...
		if not source.getPath().endswith(".ts"):
			self.session.open(MessageBox,text = _("You can only burn set-top-box recordings!"), type = MessageBox.TYPE_ERROR)
			return None
	...
	def titleEditDone(self, cutlist):
		...
		if t.VideoType != 0:
			self.session.openWithCallback(self.DVDformatCB,MessageBox,text = _("The DVD standard doesn't support H.264 (HDTV) video streams. Do you want to create a set-top-box format data DVD (which will not play in stand-alone DVD players) instead?"), type = MessageBox.TYPE_YESNO)
		else:
			self.updateTitleList()
