...
#		print job_manager.active_jobs, job_manager.failed_jobs, job_manager.job_classes, job_manager.in_background, job_manager.active_job
	...
	def showHint(self, ret=None):
		self.session.open(MessageBox, _("To update your set-top-box firmware, please follow these steps:\n1) Turn off your box with the rear power switch and make sure the bootable USB stick is plugged in.\n2) Turn mains back on and hold the DOWN button on the front panel pressed for 10 seconds.\n3) Wait for bootup and follow instructions of the wizard."), type = MessageBox.TYPE_INFO)
		self.umountCallback = self.keyRed
		self.umount()
	...
	def feed_failed(self, message=""):
		self["status"].text = _("Could not connect to set-top-box .NFI Image Feed Server:") + "\n" + str(message) + "\n" + _("Please check your network settings!")
	...
	def askStartWizard(self):
		...
		message = _("""This plugin creates a USB stick which can be used to update the firmware of your set-top-box without the need for a network or WLAN connection.
