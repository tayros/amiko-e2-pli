...
class UpdatePluginMenu(Screen):
	...
	def __init__(self, session, args = 0):
		...
		if self.menu == 0:
			...
			self.list.append(("install-extensions", _("Manage extensions"), _("\nManage extensions or plugins for your set-top-box" ) + self.oktext, None))
			self.list.append(("software-update", _("Software update"), _("\nOnline update of your set-top-box software." ) + self.oktext, None))
			self.list.append(("software-restore", _("Software restore"), _("\nRestore your set-top-box with a new firmware." ) + self.oktext, None))
			self.list.append(("system-backup", _("Backup system settings"), _("\nBackup your set-top-box settings." ) + self.oktext + "\n\n" + self.infotext, None))
			self.list.append(("system-restore",_("Restore system settings"), _("\nRestore your set-top-box settings." ) + self.oktext, None))
	...
	def checkTraficLight(self):
		...
		if default:
			...
			message += _("Do you want to update your set-top-box?")+"\n"+_("After pressing OK, please wait!")
...
class PluginManager(Screen, DreamInfoHandler):
	...
	def runExecuteFinished(self):
		...
		if plugins.restartRequired or self.restartRequired:
			self.session.openWithCallback(self.ExecuteReboot, MessageBox, _("Install or remove finished.") +" "+_("Do you want to reboot your set-top-box?"), MessageBox.TYPE_YESNO)
		else:
			self.selectedFiles = []
			self.restartRequired = False
		...
		if self.cmdlist is not None:
			for entry in self.cmdlist:
				...
				if cmd == 0:
					...
					info = _("set-top-box software because updates are available.")
...
class PluginDetails(Screen, DreamInfoHandler):
	...
	def runUpgradeFinished(self):
		self.reloadPluginlist()
		if plugins.restartRequired or self.restartRequired:
			self.session.openWithCallback(self.UpgradeReboot, MessageBox, _("Installation finished.") +" "+_("Do you want to reboot your set-top-box?"), MessageBox.TYPE_YESNO)
		else:
			self.close(True)
	...
	def ipkgCallback(self, event, param):
		if event == IpkgComponent.EVENT_DOWNLOAD:
			...
			if self.updating:
				...
				self.activityTimer.stop()
				self.activityslider.setValue(0)
				error = _("your set-top-box might be unusable now. Please consult the manual for further assistance before rebooting your set-top-box.")
				if self.packages == 0:
					error = _("No packages were upgraded yet. So you can check your network and try again.")
				if self.updating:
					error = _("Your set-top-box isn't connected to the internet properly. Please check it and try again.")
	...
	def exit(self):
		if not self.ipkg.isRunning():
			if self.packages != 0 and self.error == 0:
				self.session.openWithCallback(self.exitAnswer, MessageBox, _("Upgrade finished.") +" "+_("Do you want to reboot your set-top-box?"))
			else:
				self.close()
		else:
...
class PacketManager(Screen, NumericalTextInput):
	...
	def runRemoveFinished(self):
		self.session.openWithCallback(self.RemoveReboot, MessageBox, _("Remove finished.") +" "+_("Do you want to reboot your set-top-box?"), MessageBox.TYPE_YESNO)
	...
	def runUpgradeFinished(self):
		self.session.openWithCallback(self.UpgradeReboot, MessageBox, _("Upgrade finished.") +" "+_("Do you want to reboot your set-top-box?"), MessageBox.TYPE_YESNO)
